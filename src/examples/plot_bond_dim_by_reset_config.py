...
def plot_branch_bond_dims(ansatz, params, h, max_ancillas=6, prob_floor=1e-9, min_singular_value=1e-6):
    ...
    src/diagnostics/reset_branches.py) and plot each branch's probability
    against its max MPS bond dimension.

    Skips cleanly (no plot) if this ansatz has no reset ancillas, or if it has
    more than max_ancillas: full branch enumeration is O(2**nancillas), so
    this is only feasible for small reset configurations.
    """
    if not getattr(ansatz, "use_prob_resets_ansatz", False) or ansatz.nancillas == 0:
        print(f"h={h}: no reset ancillas on this ansatz; skipping branch bond-dim plot.")
        return

    if ansatz.nancillas > max_ancillas:
        print(f"h={h}: {ansatz.nancillas} ancillas > max_ancillas={max_ancillas}; "
              f"full branch enumeration is O(2**{ansatz.nancillas}), skipping. "
              f"(Reduce reset_layers / lattice size, or raise max_ancillas at your own risk.)")
        return
    ...
    branch_results, total_p = enumerate_branches(
        ...
        max_ancillas=max_ancillas, prob_floor=prob_floor, min_singular_value=min_singular_value,
    )

    xs = sorted(branch_results.keys())
    p_vals = [branch_results[x]["p_x"] for x in xs]
    max_bonds = [branch_results[x]["max_bond"] for x in xs]

    plt.figure(figsize=(5, 4))
    plt.scatter(p_vals, max_bonds)
    for p, b, x in zip(p_vals, max_bonds, xs):
        plt.annotate("".join(map(str, x)), (p, b), fontsize=7, xytext=(3, 3), textcoords="offset points")

    plt.xlabel("Branch probability $p_x$")
    plt.ylabel("Max bond dimension")
    plt.title(f"Reset-branch bond dims, h={h}, {Lx}x{Ly}, nlayers:{nlayers}\n(sum p_x = {total_p:.4f})")
    plt.tight_layout()
    plt.grid(visible=True, which="both", linestyle="--")

    os.makedirs(outdir, exist_ok=True)
    fname = os.path.join(outdir, f"branch_bond_dims_h_{h}.png")
    plt.savefig(fname, dpi=200)
    plt.close()
    print(f"Saved branch bond-dim plot to: {fname}")
...
# Plot each reset-branch's probability against its max MPS bond dimension
# (see src/diagnostics/reset_branches.py). Only feasible when the ansatz has
# a small enough number of reset ancillas (nancillas = 2 * total_resets):
# full enumeration is O(2**nancillas), so this is skipped with a printed
# message rather than run when nancillas > max_ancillas_for_branch_plot.
...
if __name__ == "__main__":
    ...
    if plot_branch_bond_dims_flag:
        for h in h_list:
            ansatz_h = results[h]["ansatz"]
            # final_purity is actually the final trained parameters per trial
            # (shape (trials, nparams)), despite the name -- see optimize()'s
