#!/bin/bash
#SBATCH --job-name=lightcone_reset_cmp
...
# Cheap diagnostic (small toy lattice, 4 short runs, no training loop) - a
# GPU isn't required, but is requested by default since the venv's jaxlib
# build may expect one. Drop this line (or override at submit time) if you'd
# rather run it on a CPU-only node:
#   sbatch --gres=gpu:0 run_lightcone_reset_layers_comparison.sh
...
# --- Run code ---
python -m src.examples.lightcone_reset_layers_comparison
EXIT_CODE=$?
